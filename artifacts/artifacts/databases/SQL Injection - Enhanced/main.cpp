...
// ENHANCEMENT: safe parameterized query using a prepared statement
bool run_query_safe(sqlite3* db, const std::string& name, std::vector< user_record >& records)
{
  records.clear();
  ...
  if (result != SQLITE_OK)
  {
    std::cout << "Failed to prepare safe query. ERROR = " << sqlite3_errmsg(db) << std::endl;
    return false;
  }
  ...
  if (result != SQLITE_OK)
  {
    std::cout << "Failed to bind parameter. ERROR = " << sqlite3_errmsg(db) << std::endl;
  ...
  while ((result = sqlite3_step(stmt)) == SQLITE_ROW)
  {
    ...
    records.push_back(std::make_tuple(id, userName, password));
  }

  if (result != SQLITE_DONE)
  {
    std::cout << "Error running safe query. ERROR = " << sqlite3_errmsg(db) << std::endl;
    sqlite3_finalize(stmt);
    return false;
  }
...
void run_queries(sqlite3* db)
{
  ...
  // enhancement test: safe query using prepared statement
  std::string unsafeInput = "Fred' OR 1=1;";
  if (run_query_safe(db, unsafeInput, records))
  {
    dump_results("SAFE QUERY WITH PARAMETER BINDING", records);
  }
}
